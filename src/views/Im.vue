<template>
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        // // 从本地存储中获取名为 'formData' 的数据
         const formData = localStorage.getItem('formData');
        // // 如果获取到了数据
         if (formData) {
        //   // 将获取到的数据解析为 JSON 格式，并赋值给组件的 formData 属性
           this.formData = JSON.parse(formData);}

        this.user = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')) : {}//改成从cookie中取用户
        ...
        if (typeof (WebSocket) == "undefined") {
          ...
          console.log("您的浏览器支持WebSocket");
          let socketUrl = "ws://localhost:8000/imserver/" + username;//!!!!把这个端口号写成application里的
          if (socket != null) {
            socket.close();
            socket = null;
          }
          ...
          socket.onmessage = function (msg) {
            ...
            if (data.users) {  // 获取在线人员信息
              _this.users = data.users.filter(user => user.name !== username)  // 获取当前连接的所有用户信息，并且排除自身，自己不会出现在自己的聊天列表里
            } else {
